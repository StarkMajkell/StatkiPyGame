...
def animacjaobrotutureta(góra, prawo, dół, lewo, lewo_góra, prawo_góra, prawo_dół, lewo_dół,kierunkowax,kierunkoway):
    if kierunkoway == 0:
        if kierunkowax <= 0:
            i.image = lewo  # wlewo
        else:
                ...
                if self.energia > 0:
                    ...
                    my_missile_list.append(
                        Projectile(self.x + self.pozycjaxshot, self.y + self.pozycjayshot, self.predkoscxshot,
                                   ...
                                   self.rodzajpocisku,self.dmg,0,0))
...
class Projectile():
    def __init__(self, x, y, vx, vy, rozrzut, rodzajpocisku,dmg,range,typpocisku):
        self.x = x
        self.y = y
        ...
        self.zasieg=0
        self.range=range
        self.typpocisku=typpocisku
        self.target=0
        self.rakietonspeed = 30

    def missile(self):
        if (self.istnieje == 1):
            if self.typpocisku==0:
                self.x += self.vx
                self.y += self.vy
            if self.typpocisku==1:
                self.x += self.vx
                self.y += self.vy
            obraz.blit(self.image, [self.x, self.y])
...
class Odrzut():
    def __init__(self,pozycjax,pozycjay,jakdługowisi,drganiax,drganiay,listaobrazów,któryobraz):
        self.x = pozycjax
        self.y = pozycjay
        self.czas = jakdługowisi
        self.drganiax = drganiax
        self.drganiay = drganiay
        self.image = listaobrazów[któryobraz]
        self.istnieje=1

    def update(self):
        self.x += self.drganiax
        self.y += self.drganiay
        if self.czas > 0:
            self.czas -= 1
        else:
            self.istnieje =0
...
class animacjaeksplozji():
    def __init__(self,pozycjax,pozycjay,listaanimacjieksplozji):
        self.x =pozycjax
        self.y = pozycjay
        self.iloscklatek = len(listaanimacjieksplozji)
        self.klatka = 0
        self.image1 = listaanimacjieksplozji[0]
    ...
    def update(self):
        if self.klatka ==0:
            obraz.blit(self.image1,[self.x,self.y])
        if self.klatka ==1:
            obraz.blit(self.image2,[self.x,self.y])
        if self.klatka ==2:
...
while True:
    ...
    if opcjemenu == 5:
        listagraczy=[]
        listagraczy.append(Player(600, 350, ludz,1))
        listagraczy.append(Player(600, 350, ludz,2))
        lista_efektów = []
        while True:
            ...
            for b in listagraczy:
                ...
                if b.player==1:
                    ...
                    if b.wlocie==1:
                        b.skok()
                    if pygame.key.get_pressed()[pygame.K_i] and b.kopie ==0 and b.bije==0and b.kuca==1:
                        b.kopie=1
                    if b.kopie==1:
                        b.kick()
                    if pygame.key.get_pressed()[pygame.K_o] and b.kopie ==0 and b.bije==0and b.kuca==1:
                        b.bije=1
                    ...
                    if pygame.key.get_pressed()[pygame.K_m] and b.wlocie ==0 and b.kuca==1:
                        b.wlocie=1
                    if b.wlocie==1:
                        b.skok()
                    if pygame.key.get_pressed()[pygame.K_j] and b.kopie ==0 and b.bije==0and b.kuca==1:
                        b.kopie=1
                    if b.kopie==1:
                        b.kick()
                    if pygame.key.get_pressed()[pygame.K_k] and b.kopie ==0 and b.bije==0and b.kuca==1:
                        b.bije=1
                    if pygame.key.get_pressed()[pygame.K_l] and b.kopie ==0 and b.bije==0 and pygame.key.get_pressed()[pygame.K_LEFT] and b.turlasie==0:
                        b.turlasie=1
                    if pygame.key.get_pressed()[pygame.K_l] and b.kopie ==0 and b.bije==0 and pygame.key.get_pressed()[pygame.K_RIGHT] and b.turlasie==0:
                        b.turlasie=-1
                    if pygame.key.get_pressed()[pygame.K_l] and b.kopie ==0 and b.bije==0:
                        b.kuca=0
                    else:
                        b.kuca=1
                    if b.bije==1:
 ...
 #########################################################################################################
    ...
    if opcjemenu == 3:
        ...
        while True:
            ...
            for b in lista_turet:
                if b.postawiony ==1 :
                    tabMiejsTuretow.append([b.x-25,b.y-25, b.x + 75, b.y + 75])
                ...
                for i in lista_turet:
                    if zasieg(i.x, i.y, j.x, j.y, i.range) == 1:
                        if i.magazynek>=i.firerate:
                            ...
                            animacjaobrotutureta(turet2_1, turet2_2, turet2_3, turet2_4, turet2_5, turet2_6, turet2_7,turet2_8,-((i.x - j.x) + j.vx * 10),-((i.y - j.y) + j.vy * 10))
                            if i.wartosc==30:
                                my_missile_list.append(Projectile(i.x+25,i.y+25,-((i.x - j.x)*0.1)-j.vx,-((i.y - j.y)*0.1)-j.vy,random.randint(-9,9),pocisk5,i.dmg,i.range,0))
                            elif i.wartosc==40:
                                my_missile_list.append(Projectile(i.x+25,i.y+25,-(i.x - j.x)*0.05-j.vx,-(i.y - j.y)*0.05-j.vy,0,pocisk6_1,i.dmg,i.range,1))
                            elif i.wartosc==50:
                                my_missile_list.append(Projectile(i.x+25,i.y+25,-((i.x - j.x)*0.06)-j.vx,-((i.y - j.y)*0.06)-j.vy,random.randint(-9,9),pocisk5,i.dmg,i.range,0))
            ...
            for b in lista_odrzut:
                b.update()
                if b.istnieje == 0:
                    lista_odrzut.remove(b)

            for i in my_missile_list:
                i.missile()
                i.outofmap()
                if i.typpocisku != 1:
                    i.zasieg+=((i.vx**2 + i.vy**2 )**0.5)*1.1
                if i.zasieg > i.range:
                    i.istnieje=0
                for bb in lista_enemy:
                    ...
                    if i.typpocisku ==1:
                        ...
                        if i.target==bb:
                            if bb.istnieje == 0:
                                i.target=0
                            if i.rakietonspeed > 0.3:
                                i.rakietonspeed -= 0.3
                            animacjaobrotutureta(pocisk6_1, pocisk6_2, pocisk6_3, pocisk6_4, pocisk6_5, pocisk6_6, pocisk6_7,pocisk6_8, -(i.x - bb.x), -(i.y - bb.y))
                            i.vx = -(i.x - bb.x)/i.rakietonspeed
                            i.vy = -(i.y - bb.y)/i.rakietonspeed
                        if globalnytimer == 5 or globalnytimer == 3:
                            lista_odrzut.append(Odrzut(i.x+9,i.y+9,random.randint(7,11),(random.randint(-10,10)/7)+i.vx/9,(random.randint(-10,10)/10)+i.vy/9,listaodrzutów,random.randint(0,6)))
                if i.istnieje ==0:
                    my_missile_list.remove(i)
            ...
            if globalnytimer == 5:
                if lokalnytimer2 < 2:
                    ...
                    if falapierwsza < 30:
                        falapierwsza+=1
                        lista_enemy.append(Enemy(listakropekkordyx[0], listakropekkordyy[0], 1, 2000, 0, 0, 10))
                    else:
                        xdddd=0
