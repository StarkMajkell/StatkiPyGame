...
##Klasa gracza
class Statek():
    def __init__(self,pozycjax,pozycjay,nrstatku,player,pozycjaxshot,pozycjayshot,predkoscxshot,predkoscyshot,modyfikatorrozrzutu,modyfikatorrozstrzalu,hp,rodzajpocisku,speed,energia,dmg,firerate):
...
def przyciski(y):
    if 172 < mouse[0] <212 and y + rollm+rollmstatic< mouse[1] <y+40+rollm+rollmstatic:
         if click[0]:
             obraz.blit(przycisk4, [172, y +rollm+rollmstatic])
             return 1
         else:
             obraz.blit(przycisk2, [172, y +rollm+rollmstatic])
             return 0
    if 380 < mouse[0] <420 and y + rollm+rollmstatic< mouse[1] <y+40+rollm+rollmstatic:
         if click[0]:
             obraz.blit(przycisk3, [380, y +rollm+rollmstatic])
             return -1
         else:
              obraz.blit(przycisk2, [380, y +rollm+rollmstatic])
              return 0
    else:
        return 0
    ...
    if opcjemenu == 2:
        while True:
            ...
            if 212 < mouse[0] < 382:
                ...
                if click[0]:
                    startmouse = mouse[1]
                    rollm=mouse[1]+startmouse-wysokoscOkna
                else:
                    rollmstatic+=rollm
                    rollm=0
            obraz.blit(czaszka, [700, 0])
            obraz.blit(przyciskback, [szerokoscOkna - 220, wysokoscOkna - 36])
            obraz.blit(pasek, [219, -720-(+rollm+rollmstatic)/4])
            obraz.blit(staty, [178, step1+rollm + rollmstatic])
            obraz.blit(staty2, [178, step2+rollm + rollmstatic])
            obraz.blit(staty3, [178, step3+rollm + rollmstatic])
            obraz.blit(staty4, [178, step4+rollm + rollmstatic])
